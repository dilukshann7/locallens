...
import { getTripForUser, updateTripForUser } from "@/lib/planner"
import {
...
export async function GET(
  ...
  try {
    ...
    const planner = await getTripForUser(session.user.id, id)

    if (!planner) {
      return NextResponse.json({ error: "Trip not found" }, { status: 404 })
    }

    return NextResponse.json({ tripId: id, planner })
  } catch (error) {
...
export async function PUT(
  ...
  try {
    ...
    const { id } = await params
    const updatedPlanner = await updateTripForUser(session.user.id, id, planner)
